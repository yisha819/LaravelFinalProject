<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
            ...
            <a href="/leave-requests/create" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100">
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-600">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 uppercase border-b border-gray-200">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4 font-semibold text-gray-600">Type</th>
                                    <th scope="col" class="px-6 py-4 font-semibold text-gray-600">Start Date</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($leaveRequests as $leaveRequest)
                                    <tr class="bg-white hover:bg-gray-50 transition duration-150 ease-in-out">
                                        ...
                                        <td class="px-6 py-4 font-medium text-gray-900">{{ $leaveRequest->employee->full_name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 text-gray-500">{{ $leaveRequest->type }}</td>
                                        <td class="px-6 py-4 text-gray-500">{{ \Carbon\Carbon::parse($leaveRequest->start_date)->format('M d, Y') }}</td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <form action="/leave-requests/{{ $leaveRequest->id }}" method="POST" class="inline-flex space-x-2">
                                                <a href="/leave-requests/{{ $leaveRequest->id }}" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-150 p-1 rounded hover:bg-indigo-50" title="Show">
                                                ...
                                                <a href="/leave-requests/{{ $leaveRequest->id }}/edit" class="text-amber-500 hover:text-amber-700 transition-colors duration-150 p-1 rounded hover:bg-amber-50" title="Edit">
                                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-500 hover:text-red-700 transition-colors duration-150 p-1 rounded hover:bg-red-50" title="Delete" onclick="return confirm('Are you sure you want to delete this left request?');">
                                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-6 py-10 text-center text-gray-500 bg-gray-50">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <span class="text-sm font-medium text-gray-500">There are no leave requests yet.</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                        ...
                        @elseif(!method_exists($leaveRequests, 'hasPages') && $leaveRequests instanceof \Illuminate\Pagination\LengthAwarePaginator)
                            <div class="mt-4 px-2">
                                {!! $leaveRequests->links() !!}
                            </div>
                        @else
